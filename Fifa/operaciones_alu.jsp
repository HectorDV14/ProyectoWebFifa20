...
<body class="is-preload">
		...
		//ALTA JUGADOR
		if(action.equalsIgnoreCase("AltaJugador")) {
			...
			if(request.getParameter("pierna").isEmpty()) {
				mensaje = "La pierna no puede estar vacio";
			}else {
				pierna = request.getParameter("pierna");
			}
			
			if(request.getParameter("nacionalidad").isEmpty()) {
				mensaje = "La nacionalidad no puede estar vacia";
			}else {
				nacionalidad = request.getParameter("nacionalidad");
			}
			
			if(request.getParameter("altura").isEmpty()) {
				mensaje = "La altura del jugador no puede ser 0";
			}else {
				altura = Integer.parseInt(request.getParameter("altura"));
			}
			
			if(request.getParameter("cod_equipo").isEmpty()) {
				mensaje = "El codigo del equipo no existe en la base de datos";
			}else {
				cod_equipo = altura = Integer.parseInt(request.getParameter("cod_equipo"));
			}
			
			if(mensaje.isEmpty()) {
				mensaje = "El codigo del equipo no existe en la base de datos";
				cod_jugador = controladorBD.calcularCod_jugador()+1;
				...
				mensaje = "Operacion cancelada: " + mensaje;
			}
